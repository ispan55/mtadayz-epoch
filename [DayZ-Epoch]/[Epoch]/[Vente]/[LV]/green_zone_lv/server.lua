...
function(hitElement, matchingDimension)
  if (isElement(hitElement)) and (getElementType (hitElement) == "player") and (getElementID (source) == "greenzoneColshape") then
    ...
    triggerClientEvent (hitElement, "enableGodModeLV", hitElement)
    setTeam(hitElement, "Zombies")

    outputDebugString (getPlayerName(hitElement) .. " est entrer dans la zone verte (LV)")
  end
end)
...
function(leaveElement, matchingDimension)
  if (getElementType (leaveElement) == "player") and (getElementID (source) == "greenzoneColshape") then
    ...
    outputDebugString (getPlayerName(leaveElement) .. " a quitter la zone verte (LV)")
    triggerClientEvent (leaveElement, "disableGodModeLV", leaveElement)
