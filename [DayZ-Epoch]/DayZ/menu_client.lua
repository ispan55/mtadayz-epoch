...
function showClientMenuItem(arg1, arg2, arg3, arg4)
  ...
  if arg1 == "Player" then
    ...
    if getElementData(arg2, "blood") < 11900 and 1 <= getElementData(getLocalPlayer(), "Blood Bag") then
      number = number + 1
      guiSetVisible(spalteGuiImage[number], true)
      guiSetText(spalteGuiText[number], "Administrer Blood Bag")
...
if arg1 == "Dead" then
  ...
  guiSetVisible(spalteGuiImage[number],true)
  guiSetText(spalteGuiText[number],"Hide Body")
end
...
if arg1 == "Wall" and 1 <= getElementData(getLocalPlayer(), "Toolbox") then
      number = number+1
      guiSetVisible(spalteGuiImage[number],true)
      guiSetText(spalteGuiText[number],"Remove Wall")
      if number == 1 then
         guiLabelSetColor (spalteGuiText[number],50,255,50)
         setElementData(spalteGuiText[number],"markedMenuItem",true)
      end
         setElementData(spalteGuiText[number],"usedItem","Wall")  
end
  if arg1 == "Gear" then
    number = number + 1
    guiSetVisible(spalteGuiImage[number], true)
...
Press '-' or 'middle-mouse' to remove the fence!
 REQUIRED: Toolbox]], source)
      ...
      if getElementData(source,"wall") then
         showClientMenuItem("Wall")
         setElementData(getLocalPlayer(),"currentCol",source)
         setElementData(getLocalPlayer(),"loot",false)
         setNewbieInfo (true,"Wall","Press '-' or 'middle-mouse' to remove the Wall!\n REQUIRED: Toolbox",source)
         return
      end
...
function onPlayerPressMiddleMouse(key, keyState)
  if keyState == "down" then
    ...
    if itemName == "woodprotection" then
      local col = getElementData(getLocalPlayer(),"currentCol")
      local gearName = "Remove Wood Protection"
      triggerServerEvent("removewoodprotection",getLocalPlayer(),getElementData(col,"parent"))
      disableMenu()
      return
    end
    ...
    if itemName == "hidebody" then
      if getElementData(getLocalPlayer(),"Shovel") then
    ...
    if itemName == "Assault Pack (ACU)" or itemName == "Alice Pack" or itemName == "Czech Backpack" or itemName == "Coyote Backpack" or itemName == "Camping Backpack" then
      local col = getElementData(getLocalPlayer(), "currentCol")
      triggerServerEvent("onPlayerTakeItemFromGround", getLocalPlayer(), itemName, col)
      disableMenu()
      return
