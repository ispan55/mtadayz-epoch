...
snipertxd = engineLoadTXD("mods/female.txd")
engineImportTXD(snipertxd, 191)
sniperdff = engineLoadDFF("mods/female.dff", 191)
engineReplaceModel(sniperdff, 191)
snipertxd = engineLoadTXD("mods/ghillie_suit_woman.txd")
engineImportTXD(snipertxd, 94)
sniperdff = engineLoadDFF("mods/ghillie_suit_woman.dff", 94)
engineReplaceModel(sniperdff, 94)
snipertxd = engineLoadTXD("mods/civ_girl.txd")
engineImportTXD(snipertxd, 93)
sniperdff = engineLoadDFF("mods/civ_girl.dff", 93)
engineReplaceModel(sniperdff, 93)
...
-- Nouveau sac (Camping Backpack)
--[[itemTXD = engineLoadTXD("items/backpack_camping.txd")
engineImportTXD(itemTXD, 1575)
itemDFF = engineLoadDFF("items/backpack_camping.dff", 1575)
engineReplaceModel(itemDFF, 1575)--]]

-- Infection
itemTXD = engineLoadTXD ("items/antidot.txd")
engineImportTXD (itemTXD, 1919)
itemDFF = engineLoadDFF ("items/antidot.dff", 1919)
engineReplaceModel (itemDFF, 1919)
...
engineImportTXD(itemTXD, 2472)
itemDFF = engineLoadDFF("items/backpack_czech.dff", 2472)
engineReplaceModel(itemDFF, 2472)

itemTXD = engineLoadTXD("items/backpack_survival.txd")
engineImportTXD(itemTXD, 2405)
itemDFF = engineLoadDFF("items/backpack_survival.dff", 2405)
engineReplaceModel(itemDFF, 2405)


itemTXD = engineLoadTXD("items/backpack_british.txd")
engineImportTXD(itemTXD, 2406)
itemDFF = engineLoadDFF("items/backpack_british.dff", 2406)
engineReplaceModel(itemDFF, 2406)


itemTXD = engineLoadTXD("items/backpack_patrol.txd")
engineImportTXD(itemTXD, 2407)
itemDFF = engineLoadDFF("items/backpack_patrol.dff", 2407)
engineReplaceModel(itemDFF, 2407)


itemTXD = engineLoadTXD("items/backpack_vest.txd")
engineImportTXD(itemTXD, 2408)
itemDFF = engineLoadDFF("items/backpack_vest.dff", 2408)
engineReplaceModel(itemDFF, 2408)--]]

-- Nouvelle boisson
itemTXD = engineLoadTXD("items/redbull_can.txd")
engineImportTXD(itemTXD, 2215)
itemDFF = engineLoadDFF("items/redbull_can.dff", 2215)
engineReplaceModel(itemDFF, 2215)
itemTXD = engineLoadTXD("items/sard_can.txd")
engineImportTXD(itemTXD, 2216)
itemDFF = engineLoadDFF("items/sard_can.dff", 2216)
engineReplaceModel(itemDFF, 2216)
itemTXD = engineLoadTXD("items/coca_can.txd")
engineImportTXD(itemTXD, 2217)
itemDFF = engineLoadDFF("items/coca_can.dff", 2217)
engineReplaceModel(itemDFF, 2217)
itemTXD = engineLoadTXD("items/pepsi_can.txd")
engineImportTXD(itemTXD, 2218)
itemDFF = engineLoadDFF("items/pepsi_can.dff", 2218)
engineReplaceModel(itemDFF, 2218)
itemTXD = engineLoadTXD("items/limo_can.txd")
engineImportTXD(itemTXD, 2219)
itemDFF = engineLoadDFF("items/limo_can.dff", 2219)
engineReplaceModel(itemDFF, 2219)
...
function refreshDebugMonitor()
  ...
  local title1 = {fr = "Zombies tués: "}
  local title2 = {fr = "Tirs dans la tête: "}
  local title3 = {fr = "Bandits tués: "}
  local title4 = {fr = "Meurtres: "}
  local title5 = {fr = "Sang: "}
  local title6 = {fr = "Zombies (En vie/Total): "}
  local title7 = {fr = "Température: "}
  local title8 = {fr = "Humanité: "}
  local title9 = {fr = "Monnaie: "}

  if getElementData(getLocalPlayer(), "logedin") then
    local money = getPlayerMoney()
    local value = getElementData(getLocalPlayer(), getElementData(statsLabel.zombieskilled, "identifikation"))
    if title1[languageCode] then
     guiSetText(statsLabel.zombieskilled, title1[languageCode] .. value)
    else
    ...
    if title2[languageCode] then
     guiSetText(statsLabel.headshots, title2[languageCode] .. value)
    else
    ...
    if title3[languageCode] then
     guiSetText(statsLabel.banditskilled, title3[languageCode] .. value)
    else
    ...
    if title4[languageCode] then
     guiSetText(statsLabel.murders, title4[languageCode] .. value)
    else
    ...
    if title5[languageCode] then
     guiSetText(statsLabel.blood, title5[languageCode] .. value)
    else
    ...
    if title6[languageCode] then
     guiSetText(statsLabel.zombies, title6[languageCode] .. value .. "/" .. value2)
    else
    ...
    if title7[languageCode] then
     guiSetText(statsLabel.temperature, title7[languageCode] .. math.round(value, 2) .. "\194\176C")
    else
    ...
    if title8[languageCode] then
     guiSetText(statsLabel.humanity, title8[languageCode] .. math.round(value, 2))
    else
     guiSetText(statsLabel.humanity, "Humanity: " .. math.round(value, 2))
    end
    if title9[languageCode] then
     guiSetText(statsLabel.name, title9[languageCode] ..money.. " EP's")
    else
     guiSetText(statsLabel.name, "Money: " ..money.. " EP's")
    end
  end
end
...
function playerGetDamageDayZ(attacker, weapon, bodypart, loss)
  ...
  if getElementData(attacker,"zombie") then
    ...
    --[[local aleatory = math.random(1,50)
      if aleatory == 3 then
...
if getElementData(getLocalPlayer(),"logedin") then
     if getElementData(getLocalPlayer(),"infection") then
        setElementData(getLocalPlayer(),"bleeding",math.random(300,900))
     end
end 
end
setTimer(checkInfection,5000,0)
...
function setPain()
  if getElementData(getLocalPlayer(), "logedin") and getElementData(getLocalPlayer(), "pain") then
    local x, y, z = getElementPosition(getLocalPlayer())
    createExplosion(x, y, z + 22, 8, false, 1, false)
  ...
  if getElementData(getLocalPlayer(), "logedin") then
    ...
    for i, player in ipairs(getElementsByType("player")) do
      ...
      if player ~= getLocalPlayer() then
        ...
        local pdistance = getDistanceBetweenPoints3D(x, y, z, px, py, pz)
          -- Pseudo sup (fichier mise à jour MTA:DayZ)
      end
    end
    local x, y, z = getElementPosition(getLocalPlayer())
    ...
    if veh then
      ...
      --dxDrawText(tires .. "/" .. needtires .. " Tires", screenWidth * 0.5 - w / 2, screenHeight * 0 + offset, screenWidth * 0.5 - w / 2, screenHeight * 0 + offset, tocolor(r, g, b, 220), 1.02, "default-bold")
        dxDrawText(tires .. "/" .. needtires .. " Tires", screenWidth * 0.5 - w / 2, screenHeight * 0, screenWidth * 0.5 - w / 2, screenHeight * 0 + offset, tocolor(r, g, b, 220), 1.02, "default-bold")
        dxDrawText("Fuel:" .. math.floor(fuel) .. "/" .. maxfuel, screenWidth * 0.5 - w / 2, screenHeight * 0 + offset, screenWidth * 0.5 - w / 2, screenHeight * 0 + offset * 2, tocolor(r, g, b, 220), 1.02, "default-bold")
...
function infection(source)
    if getElementData(getLocalPlayer(), "logedin") then
        if getElementData(getLocalPlayer(), "infection") then
            ...
            local aleatorySound6 = math.random(1,50)
            local sound = playSound("sounds/ogg/action/cough/action_cough"..aleatorySound6..".ogg")
...
function updatePlayTime()
  if getElementData(getLocalPlayer(), "logedin") then
    ...
    setPlayerMoney(getLocalPlayer(), playermoney + math.random(50,70))
  end
end
